...
from . import DiagnosisError, DiagnosisRecord, TokenCounts, diagnose, sum_tokens
...
class Aggregate(FrozenModel):
    ...
    def context_per_call(self) -> float | None:
        context = self.tokens.input_tokens + self.tokens.cache_read_tokens
        return context / self.calls if self.calls else None

    @computed_field  # type: ignore[prop-decorator]  # Pydantic computed property
    @property
    def cache_hit_rate(self) -> float | None:
        context = self.tokens.input_tokens + self.tokens.cache_read_tokens
        return self.tokens.cache_read_tokens / context if context else None
...
class Cohort(FrozenModel):
    selections: tuple[KanbanRun, ...] = Field(min_length=1)
    ...
    def validate_attribution(self) -> Self:
        ...
        for selection, run in zip(self.selections, self.runs):
            ...
            if attribution.runtime != "kanban" or selection.run_id != attribution.run_id:
                raise ValueError("Run does not match its Kanban selection")
...
class DiagnosisReport(FrozenModel):
    ...
    measurement_method: Literal["kanban-worker-session/summed-usage/four-units-v1"]
...
def save_report(report: DiagnosisReport, store: DiagnosisStore) -> StoredReport:
    workflow = hashlib.sha256(f"kanban-report:{report.workflow_identity}".encode()).hexdigest()
...
def diagnose_report(hermes_home: Path, runs: tuple[KanbanRun, ...], workflow_identity: str,
    ...
    usage = HermesUsage(hermes_home, protected_roots=protected_roots)
    records = tuple(diagnose(
        run.run_id, KanbanAdapter(hermes_home, run.board, protected_roots=protected_roots),
        usage, store,
    ).record for run in runs)
    try:
        report = DiagnosisReport(
